    ...
    <div className="grid grid-cols-12 gap-8 h-[calc(100vh-100px)]">
      ...
      <div className="col-span-12 lg:col-span-8 flex flex-col gap-6 overflow-hidden">
        <div className="relative">
          ...
          <input 
            ref={searchRef}
            placeholder="Ketik nama parfum atau scan barcode... (Enter to select)" 
            className="w-full pl-16 pr-6 py-6 bg-white border-2 border-transparent focus:border-brand-primary outline-hidden rounded-3xl text-xl font-medium shadow-sm transition-all"
            value={searchTerm}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 pb-8">
            {filteredProducts.map(p => (
              <motion.button
                ...
                className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm hover:shadow-lg hover:border-brand-primary/20 transition-all text-left flex flex-col group active:scale-95"
              >
                <div className="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center text-brand-primary font-bold text-xl italic mb-4 group-hover:bg-brand-primary group-hover:text-white transition-colors">
                  {p.name.charAt(0)}
                </div>
                <h4 className="font-bold text-slate-800 line-clamp-2 mb-1">{p.name}</h4>
                <p className="text-slate-400 text-xs font-semibold mb-3 uppercase tracking-wider">{p.category}</p>
                <p className="mt-auto font-display font-bold text-brand-primary text-lg">
                  Rp {p.price.toLocaleString()}
                  <span className="text-xs text-slate-400 font-sans ml-1">{p.type === 'PARFUM' ? '/ml' : '/pcs'}</span>
                </p>
              </motion.button>
            ))}
          </div>
      ...
      {/* Cart Area */}
      <div className="col-span-12 lg:col-span-4 flex flex-col bg-white rounded-3xl border border-slate-100 shadow-xl overflow-hidden">
            ...
            cart.map((item, idx) => (
              <div key={`${item.id}-${idx}`} className="flex justify-between items-start group">
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  <button onClick={() => removeFromCart(idx)} className="text-brand-danger opacity-0 group-hover:opacity-100 transition-opacity">
                    <X size={14} />
                  </button>
                </div>
              </div>
        ...
        <div className="p-6 bg-slate-50 border-t border-slate-100 space-y-4">
          ...
          <div className="space-y-2 mt-4 pt-4 border-t border-slate-200">
             <div className="relative">
                ...
                <input 
                  type="number" 
                  placeholder="Tunai yang dibayar..." 
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="flex flex-col items-center text-center">
                <div className="w-20 h-20 bg-brand-primary/10 text-brand-primary rounded-3xl flex items-center justify-center text-3xl font-bold mb-4 italic">
                  {selectedProduct.name.charAt(0)}
                </div>
                <h3 className="text-2xl font-bold text-slate-800 font-display">{selectedProduct.name}</h3>
                <p className="text-slate-400 uppercase text-xs font-bold tracking-widest mt-1">{selectedProduct.category}</p>
                <div className="bg-slate-50 px-4 py-2 rounded-full mt-4 flex items-center gap-2">
                  <span className="text-slate-500 font-medium text-sm">Harga:</span>
                  <span className="text-brand-primary font-bold">Rp {selectedProduct.price.toLocaleString()}{selectedProduct.type === 'PARFUM' ? '/ml' : '/pcs'}</span>
                </div>
              </div>
              ...
              <div className="mt-8 space-y-6">
                <div className="space-y-3">
                  ...
                  <input 
                    ...
                    className="w-full px-6 py-5 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-brand-primary/10 transition-all outline-hidden text-center text-2xl font-bold text-slate-800"
                    placeholder="0"
                  />
                  {selectedProduct.type === 'PARFUM' && mlInput && (
                    ...
                    >
                      <p className="text-slate-400 text-sm">Total Harga Item:</p>
                      <p className="text-2xl font-bold text-slate-800">Rp {(parseFloat(mlInput) * selectedProduct.price).toLocaleString()}</p>
                    </motion.div>
                  )}
                </div>
                ...
                <button 
                  ...
                  className="w-full py-5 bg-brand-primary text-white font-bold rounded-2xl shadow-xl shadow-brand-primary/25 hover:bg-sky-600 transition-all text-xl"
                >
